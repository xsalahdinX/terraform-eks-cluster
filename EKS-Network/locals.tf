...
locals {
  private_az_index = element(var.az, index(tolist(toset(var.private_ciders)), each.key) % length(var.az))
  public_az_index  = element(var.az, index(tolist(toset(var.public_ciders)), each.key) % length(var.az))
}